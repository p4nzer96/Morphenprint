...
def get_filtered_minutae_list(minutiae_file_path, mask):
    ...
    """
    read_file = open(minutiae_file_path, 'r')
    minutiae_list_filtered = ''
    minutiae_count = 0
    for line in read_file:
        try:
            ...
            if mask[y][x] is False:
                minutiae_list_filtered = str(minutiae_list_filtered) + str(line)
                minutiae_count += + 1
        except Exception:
            continue

    return minutiae_count, minutiae_list_filtered
...
def get_minutiae_df(img1_cropped, img2_cropped, img1_cropped_minutiae_path, img2_cropped_minutiae_path):
    ...
    for _ in range(513):
        try:
            ...
            minutae_count_img1_1, minutae_count_img2_2, minutae_list_filtered_img1_1, minutae_list_filtered_img2_2 = get_filtered_minutae_info_of_images(
            ...
            minutae_count_img1_2, minutae_count_img2_1, minutae_list_filtered_img1_2, minutae_list_filtered_img2_1 = get_filtered_minutae_info_of_images(
        ...
        for root, _, files in os.walk(directory_path):
            ...
            if img1_cropped_path and img2_cropped_path and img1_minutiae_path and img2_minutiae_path:
                if os.path.getsize(img1_minutiae_path) == 0 or os.path.getsize(img2_minutiae_path) == 0:
                    with open(data_txt_path, 'a') as file:
                        file.write('\n' + 'Image Sets - ' + str(os.path.basename(root)) + ',' + str(
                            os.path.basename(img1_cropped_path)) + ',' + str(os.path.basename(img2_cropped_path)))
                    continue
                ...
                morphed_minutiae_save_path = root + '/' + str(
                    ...
                    os.path.splitext(os.path.basename(img2_cropped_path))[0]) + '_mm.txt'
