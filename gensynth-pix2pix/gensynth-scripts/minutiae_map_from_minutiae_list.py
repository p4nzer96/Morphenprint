...
def create_minutiaeMap(minutiae_txt_dir, minutiae_map_dir, scale = 1, method = 'pointingMinutiae', size = (512,512), file_name=None):
    """Create minutia map from a list of minitiae by depicting minutiae as black squares on a white background. 
    There is no destinction between ridge line endings and bifurcations. 
...
def main():
    ...
    if ( len(sys.argv) < 3 ):
        print('Usage: python '+sys.argv[0]+ ' <minutiae_dir> <output_dir> <file_name> [method] [downscale]>')
    ...
    create_minutiaeMap(minutiae_dir, output_dir, scale, method, size, file_name)
