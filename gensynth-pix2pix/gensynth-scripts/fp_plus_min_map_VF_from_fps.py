...
def create_minutiae_map(input_dir, output_dir, method, scale, is_double_image, file_name=None):
    """ Extracts minutiae information from fingerprints.
    Creates Minutiae Map. 
    """
    fp_image_files = None
    ...
    for idx, fp_image_file in enumerate(fp_image_files):
        ...
        # save minutiae map and also check the double image flag
        if is_double_image == 'yes':
            ...
            minutiae_map.save(output_dir+'/' + fp_image_file[::-1].split('.', 1)[1][::-1] + '.png')
...
def main():
    ...
    if ( len(sys.argv) < 3 ):
        ...
        print('\tfile_name: name of the file to be processed (default=all files in the input folder)')
        print('\tis_double_image: creates a double image for pix2pix (default=yes)')
        print('\tmethod: monoSquare | graySquare | pointingMinutiae | directedMinutiae | patchSkeleton (default=directedMinutiae)')
    ...
    if len(sys.argv) > 5:
        ...
        method = 'directedMinutiae'

    if len(sys.argv) > 6:
        scale = 1.0 / float(sys.argv[6])
    else:
        scale = 1.0

    create_minutiae_map(input_dir, output_dir, method, scale, is_double_image, file_name)
