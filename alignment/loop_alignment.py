...
def get_good_loop_align(img_t, W, img2_center, loop_list_img1, angles_img1, rel_img1):
    ...
    for angle in np.arange(start_angle, end_angle + 1, angle_step):
        try:
            ...
            # Translate the image
            img2_t_r_t = translate(
                img2_t_r,
                loop_list_img1[0][0][1] - img2_t_r_loop_list[0][0][1],
                loop_list_img1[0][0][0] - img2_t_r_loop_list[0][0][0],
            )
            # Get the loop list, angles and reliability of the translated image
            _, angles_img2_t_r_t, rel_img2_t_r_t = get_loop_list_angles_rel_img(
                img2_t_r_t, W
            )
            # Calculate the similarity score
            similarity_score = calculate_similarity(
                angles_img1, angles_img2_t_r_t, rel_img1, rel_img2_t_r_t
            )
            # Append the similarity score to the dataframe
            sim_score_loop_df = pd.concat(
                [
                    sim_score_loop_df,
                    pd.DataFrame(
                        [
                            {
                                ...
                                "tx": loop_list_img1[0][0][1]
                                - img2_t_r_loop_list[0][0][1],
                                "ty": loop_list_img1[0][0][0]
                                - img2_t_r_loop_list[0][0][0],
    ...
    for value in np.arange(start_t, end_t, t_step):
        ...
        for angle in np.arange(start_angle, end_angle + 1, angle_step):
            try:
                ...
                sim_score_loop_df_final = pd.concat(
                    [
                        sim_score_loop_df_final,
                        pd.DataFrame(
                            [
                                {
                                    "rotation_angle": angle,
...
def get_loop_sim_score_df(
    img2, W, angles_img1, rel_img1, tx_loop, ty_loop, loop_list_img1
):
    """
    ...
    try:
        ...
        sim_score_loop_df = get_good_loop_align(
            img2_t, W, img2_center, loop_list_img1, angles_img1, rel_img1
        )
        max_loop_sim_score = sim_score_loop_df[
            sim_score_loop_df["similarity_score"]
            == sim_score_loop_df["similarity_score"].max()
        ]
        img2_t_r = rotate(
            img2_t, int(max_loop_sim_score["rotation_angle"]), img2_center
        )
        img2_t_r_t = translate(
            img2_t_r, int(max_loop_sim_score["tx"]), int(max_loop_sim_score["ty"])
        )

        sim_score_loop_df_final = get_best_loop_align(
            img2_t_r_t, W, angles_img1, rel_img1, img2_center
        )
