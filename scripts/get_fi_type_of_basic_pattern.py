        ...
        for file in tqdm(os.listdir(self.input_directory)):
            try:
                if file.endswith(".png"):
                    ...
                    smooth_angles_img, _, _, _= orientation.calculate_angles(img, self.block_size, smoth=True)
                    _, _, mask = segmentation.create_segmented_and_variance_images(img, self.block_size, 0.2)
                    singularities = Singularities(img, smooth_angles_img, 1, self.block_size, mask)
