...
def get_loop_list_angles_rel_img(img, block_size):
  smooth_angles_img_t, angles_img_t, rel_img_t = fi_orientation.calculate_angles(img, block_size, smoth=True)
  _, _, mask_t = fi_segmentation.create_segmented_and_variance_images(img, block_size, 0.2)
  _, _, loop_list_t, _, _ = fi_singularity.calculate_singularities(img, smooth_angles_img_t, 1, block_size, mask_t)
  return loop_list_t, angles_img_t, rel_img_t
...
def get_good_rotation_translation_config(img_t, loop_difference_x, block_size, img2_w_center, img2_h_center, loop_list_img1, angles_img1, rel_img1):
    ...
    for angle in np.arange(angle_value_from, angle_value_to, 1):
        ...
        img2_t_r_loop_list, _, _ = get_loop_list_angles_rel_img(img2_t_r, block_size)
        ...
        _, angles_img2_t_r_t, rel_img2_t_r_t = get_loop_list_angles_rel_img(img2_t_r_t, block_size)
...
def get_best_whorl_fi_alignment_config(img2, block_size, angles_img1, rel_img1, sim_score_whorl_df, img2_w_center, img2_h_center):
...
def get_whorl_fi_sim_score_df(block_size, img2, angles_img1, rel_img1, tx_whorl, ty_whorl, loop_list_img1):
    ...
    img2_t_loop_list, _, _ = get_loop_list_angles_rel_img(img2_t, block_size, img2_w_center, img2_h_center, loop_list_img1, angles_img1, rel_img1)
    loop_difference_x = loop_list_img1[2][0][1] - img2_t_loop_list[2][0][1]
    sim_score_whorl_df = get_good_rotation_translation_config(img2_t, loop_difference_x, block_size)
    ...
    sim_score_whorl_df_final = get_best_whorl_fi_alignment_config(img2_t_r_t, block_size, angles_img1, rel_img1, img2_h_center, img2_w_center, sim_score_whorl_df_final)
