...
def get_best_loop_fi_alignment_config(img2, block_size, angles_img1, rel_img1, sim_score_loop_df):
    ...
    for value in np.arange(-5, 6, 1):
        ...
        for angle in np.arange(-5, 6, 1):
            ...
            sim_score_loop_df = sim_score_loop_df.append({'rotation_angle': angle, 'tx': value, 'ty': value, 'similarity_score': similarity_score}, ignore_index=True)

    return sim_score_loop_df

def get_loop_fi_sim_score_df(block_size, img2, angles_img1, rel_img1, tx_loop, ty_loop):
    try:
