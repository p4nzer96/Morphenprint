...
def get_x_y_coordinates(directory_path):
    ...
    for value in X:
        y_m_count, y_a_count = 0, 0
        for _, line in enumerate(lines):
            try:
                if len(line) > 0 and line[0] == '{':
                    X_F1_matching_score = float(line[
                                                line.find("Morph_Img1_score=") + len("Morph_Img1_score="): line.find(
                                                    ", Morph_Img2_score=")])

                    Y_F2_matching_score = float(line[
                                                line.find("Morph_Img2_score=") + len("Morph_Img2_score="): line.find(
                                                    ", Morph_Img3_score=")])
                    Z_F2_matching_score = float(
