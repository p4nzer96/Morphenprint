        ...
        try:
            if len(line) > 0 and line[0] == "{":
                X_F1_matching_score = float(
                    line[
                        line.find("Morph_Img1_score=")
                        + len("Morph_Img1_score=") : line.find(", Morph_Img2_score=")
                    ]
                )
                Y_F2_matching_score = float(
                    line[
                        line.find("Morph_Img2_score=")
                        + len("Morph_Img2_score=") : line.find(", Morph_Img3_score=")
                    ]
                )
                Z_F3_matching_score = float(
                    line[
                        line.find("Morph_Img3_score=")
                        + len("Morph_Img3_score=") : line.find("}")
                    ]
                )
...
def main():
    ...
    for i in range(len(x)):
        if x[i] > 36 and y[i] > 36 and z[i] > 36:
            if x[i] > 60 and y[i] > 60 and z[i] > 60:
                above_36_points.append(
                    ax.scatter(x[i], y[i], z[i], color="green", marker="D", s=60)
                )

            elif x[i] > 48 and y[i] > 48 and z[i] > 48:
                above_36_points.append(
                    ax.scatter(x[i], y[i], z[i], color="green", marker="s", s=40)
                )

            else:
                above_36_points.append(
                    ax.scatter(x[i], y[i], z[i], color="green", marker="o", s=20)
                )

        if x[i] < 36 and y[i] < 36 and z[i] < 36:
            below_36_points.append(
                ax.scatter(x[i], y[i], z[i], color="red", marker="o", s=15)
            )
    ...
    custom_legend = [
        ax.scatter(
            [], [], color="green", marker="D", s=10, label="Triple-Identity FPs > DT3"
        ),
        ax.scatter(
            [],
            [],
            ...
            marker="s",
            s=10,
            label="DT3 > Triple-Identity FPs > DT2",
        ),
        ax.scatter(
            [],
            [],
            ...
            marker="o",
            s=10,
            label="DT2 > Triple-Identity FPs > DT1",
        ),
        ax.scatter([], [], color="red", marker="o", s=10, label="Virtual-Identity FPs"),
        ax.scatter(
            [], [], color="orange", marker="o", s=10, label="Partial-Identity FPs"
        ),
    ]
