"""
To eliminate the edges of the image and areas that are too noisy, segmentation is
necessary. It is based on the calculation of the variance of gray levels. For this purpose, the image
is divided into sub-blocks of (W x W) size and for each block the variance.
...
def create_segmented_and_variance_images(im, w, threshold=.2):
    ...
    It also normalizes the intensity values of the image so that the ridge regions have zero mean, unit standard
    deviation.

    Args:
    im (numpy.ndarray): Fingerprint image.
    w (int): Size of the block.
